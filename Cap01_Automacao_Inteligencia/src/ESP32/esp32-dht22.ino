...
// Declaração global do LCD
LiquidCrystal_I2C lcd(0x27, 16, 2); // Endereço 0x27, 16 colunas e 2 linhas

// Variável para controlar o estado anterior da bomba
...
void atualizarLCD(bool bombaLigada) {
    lcd.clear();
    lcd.setCursor(0, 0);
...
void loop() {
    ...
    // Atualiza o LCD apenas se o estado da bomba mudou
    if (bomba_estado != estadoAnteriorBomba) {
        atualizarLCD(bomba_estado);
        estadoAnteriorBomba = bomba_estado;
    }
